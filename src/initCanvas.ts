...
export function renderLayers(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
  ...
  for (const layer of layersBottomFirst) {
    ...
    if (backgroundLayer && layer.id === backgroundLayer.id) {
      ...
      // Draw other layers at their intrinsic size at (0,0)
...
function applyResize(handle: HandleId, dx: number, dy: number, rect: { x: number; y: number; w: number; h: number }) {
  ...
  if (w < minSize) { const diff = minSize - w; if (handle === 'nw' || handle === 'sw') x -= diff; w = minSize; }
  if (h < minSize) { const diff = minSize - h; if (handle === 'nw' || handle === 'ne') y -= diff; h = minSize; }
...
export function initCanvas(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
  ...
  canvas.addEventListener('pointerdown', (e) => {
    ...
    for (const layer of layersTopFirst) {
      ...
      if (w <= 0 || h <= 0) continue;
      if (pointInRect(p.x, p.y, layer.x, layer.y, w, h)) {
        picked = { ...layer, w, h };
        // use the first match (top-most first order)
        selectedId = layer.id;
        break;
      }
    }
    ...
    if (!picked) {
      // If clicked on empty space, keep selection but do nothing
      renderLayers(canvas, ctx);
      return;
    }

    // Determine if pointer is on a resize handle
    const handle = getHandleAt(picked, p.x, p.y);
    if (handle) {
      dragMode = 'resize';
      activeHandle = handle;
    } else {
    ...
    startRect = { x: picked.x, y: picked.y, w: picked.w, h: picked.h };
  ...
  canvas.addEventListener('pointermove', (e) => {
    ...
    if (!dragMode) {
      ...
      for (const l of layers) {
        ...
        const w = l.w && l.w > 0 ? l.w : l.image?.width ?? 0;
        const h = l.h && l.h > 0 ? l.h : l.image?.height ?? 0;
        if (w <= 0 || h <= 0) continue;
        if (!pointInRect(p.x, p.y, l.x, l.y, w, h)) continue;
        const hId = getHandleAt({ x: l.x, y: l.y, w, h }, p.x, p.y);
        if (hId) {
